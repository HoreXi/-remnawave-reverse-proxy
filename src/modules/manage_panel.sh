...
start_panel_node() {
    ...
    if docker ps -q --filter "ancestor=remnawave/backend:latest" | grep -q . || docker ps -q --filter "ancestor=remnawave/node:latest" | grep -q . || docker ps -q --filter "ancestor=remnawave/node:2.8.0" | grep -q . || docker ps -q --filter "name=remnanode" | grep -q . || docker ps -q --filter "ancestor=remnawave/backend:2" | grep -q .; then
