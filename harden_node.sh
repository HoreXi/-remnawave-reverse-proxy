#!/bin/bash
# ==============================================================================
# Remnawave Node Hardening & Optimization Suite (Standalone)
# Features:
#  1. Geosite.dat + Cron auto-update
#  2. Clean UFW configuration (SSH 22222, 443 TCP/UDP, 80 TCP, 2222 panel only)
#  3. Nginx server_tokens off + Sysctl (BBR) + Fail2ban
#  4. SSH moved to port 22222 + Public key only (lightbeam-vps)
#  5. Accurate time synchronization (Chrony / NTP)
#  6. Automatic security patches (Unattended-Upgrades)
#  7. Fail2ban jail for SSH on port 22222
#  8. SSH daemon limits and hardening
#  9. Hide OS banners (/etc/issue, /etc/issue.net)
# 10. Memory hardening, DNS 1.1.1.1, Network blacklist, Docker prune cron
# 11. Lock root password hash (passwd -l root)
# 12. Free disk space monitor in Cron (>90% Docker prune)
# 13. Restrict port 2222 strictly to Master Panel IP (45.39.241.238)
# 14. Master Security Check & iperf3 cleanup
# ==============================================================================
...
if [ "$EUID" -ne 0 ]; then
    echo -e "${COLOR_RED}Ошибка: Этот скрипт необходимо запускать с правами root (sudo)!${COLOR_RESET}"
    exit 1
fi
...
harden_geosite_cron() {
    ...
    echo -e "\n${COLOR_YELLOW}[1/13] Настройка Geosite.dat и автообновления в Cron...${COLOR_RESET}"
...
harden_ufw_firewall() {
    ...
    echo -e "\n${COLOR_YELLOW}[2/13] Чистая настройка UFW для Ноды (Мастер-панель IP: ${panel_ip})...${COLOR_RESET}"
...
harden_nginx_and_kernel() {
    echo -e "\n${COLOR_YELLOW}[3/13] Скрытие версии Nginx + Тюнинг ядра (Sysctl/BBR) + Fail2ban...${COLOR_RESET}"
...
harden_ssh() {
    echo -e "\n${COLOR_YELLOW}[4/13] Перевод SSH на порт 22222 + Вход строго по Ключам + Тюнинг демона SSH...${COLOR_RESET}"
...
harden_chrony() {
    echo -e "\n${COLOR_YELLOW}[5/13] Настройка синхронизации времени Chrony (NTP)...${COLOR_RESET}"
...
harden_unattended_upgrades() {
    echo -e "\n${COLOR_YELLOW}[6/13] Настройка автоматических патчей безопасности (Unattended-Upgrades)...${COLOR_RESET}"
...
harden_fail2ban_sshd() {
    echo -e "\n${COLOR_YELLOW}[7/13] Конфигурация Fail2ban для SSH (порт 22222)...${COLOR_RESET}"
...
# 9. Hide OS banners
harden_hide_banners() {
    echo -e "\n${COLOR_YELLOW}[8/13] Скрытие системных баннеров ОС (/etc/issue)...${COLOR_RESET}"
...
harden_additional_security() {
    echo -e "\n${COLOR_YELLOW}[9/13] Дополнительная защита: память, DNS, blacklist протоколов, Docker prune...${COLOR_RESET}"
...
harden_lock_root_password() {
    echo -e "\n${COLOR_YELLOW}[10/13] Блокировка парольной аутентификации root на уровне системы...${COLOR_RESET}"
...
harden_disk_monitor_cron() {
    echo -e "\n${COLOR_YELLOW}[11/13] Настройка контроля свободного места на диске в Cron (>90%)...${COLOR_RESET}"
...
harden_restrict_panel_port() {
    local panel_ip="${1:-$HARDENING_DEFAULT_PANEL_IP}"
    echo -e "\n${COLOR_YELLOW}[12/13] Ограничение доступа к порту 2222 только для Master Panel IP (${panel_ip})...${COLOR_RESET}"
...
# Run
PANEL_IP="${1:-$HARDENING_DEFAULT_PANEL_IP}"
apply_all "$PANEL_IP"
